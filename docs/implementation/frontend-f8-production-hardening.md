...
- Added static package module fallback for `/packages/**/*.js` so compiled web modules can import shared contracts from `dist/packages`.
...
- The second browser verification still showed a blank page because compiled web modules import shared contracts via `/packages/contracts/src/index.js`, which also returned 404. The dev server now serves compiled package modules from `dist/packages`.
